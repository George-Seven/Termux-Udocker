...
if [ -n "$1" ]; then
  udocker_run --entrypoint "bash -c" -p "$PORT:8123" -e _PORT="$PORT" -e TZ="$(getprop persist.sys.timezone)" -v "$(proot_write_tmp "$(cat "$(pwd)/libnetstub.sh")"):/.libnetstub/libnetstub.sh" -v "$(mktemp):/proc/sys/net/ipv4/ip_forward" -v "$DATA_DIR/config:/config" "$CONTAINER_NAME" "$@"
else
  udocker_run --entrypoint "bash -c" -p "$PORT:8123" -e _PORT="$PORT" -e TZ="$(getprop persist.sys.timezone)" -v "$(proot_write_tmp "$(cat "$(pwd)/libnetstub.sh")"):/.libnetstub/libnetstub.sh" -v "$(mktemp):/proc/sys/net/ipv4/ip_forward" -v "$DATA_DIR/config:/config" "$CONTAINER_NAME" ' \
