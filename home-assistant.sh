...
if [ -n "$1" ]; then
  unset cmd
  cmd="$*"
  udocker_run --entrypoint "bash -c" -p "$PORT:8123" -e _PORT="$PORT" -e TZ="$(get_tz)" -v "$(mktemp):/proc/sys/net/ipv4/ip_forward" -v "$DATA_DIR/config:/config" "$CONTAINER_NAME" "$cmd"
else
  udocker_run --entrypoint "bash -c" -p "$PORT:8123" -e _PORT="$PORT" -e TZ="$(get_tz)" -v "$(mktemp):/proc/sys/net/ipv4/ip_forward" -v "$DATA_DIR/config:/config" "$CONTAINER_NAME" ' \
